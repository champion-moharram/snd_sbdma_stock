...
erpnext.accounts.bank_reconciliation.DataTableManager = class DataTableManager {
	...
	make_dt() {
		var me = this;
		frappe.call({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.bank_reconciliation_tool.bank_reconciliation_tool.get_bank_transactions",
			args: {
	...
	get_cleared_balance() {
		if (this.bank_account && this.bank_statement_to_date) {
			return frappe.call({
				method: "snd_sbdma_stock.snd_sbdma_stock.doctype.bank_reconciliation_tool.bank_reconciliation_tool.get_account_balance",
				args: {
					bank_account: this.bank_account,
