...
{% include 'snd_sbdma_stock/snd_sbdma_stock/doctype/purchase_taxes_and_charges_template/purchase_taxes_and_charges_template.js' %}
...
erpnext.buying.BuyingController = class BuyingController extends erpnext.TransactionController {
	...
	setup_queries(doc, cdt, cdn) {
		...
		this.frm.set_query("manufacturer", "items", function(doc, cdt, cdn) {
			...
			return {
				query: "snd_sbdma_stock.controllers.queries.item_manufacturer_query",
	...
	warehouse(doc, cdt, cdn) {
		...
		if(item.item_code && item.warehouse) {
			return this.frm.call({
				method: "snd_sbdma_stock.snd_sbdma_stock.get_item_details.get_bin_details",
				child: item,
				args: {
	...
	manufacturer(doc, cdt, cdn) {
		...
		if(row.manufacturer) {
			frappe.call({
				method: "snd_sbdma_stock.snd_sbdma_stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
				args: {
...
erpnext.buying.get_default_bom = function(frm) {
	$.each(frm.doc["items"] || [], function(i, d) {
		if (d.item_code && d.bom === "") {
			return frappe.call({
				type: "GET",
				method: "snd_sbdma_stock.snd_sbdma_stock.get_item_details.get_default_bom",
				args: {
...
erpnext.buying.get_items_from_product_bundle = function(frm) {
	var dialog = new frappe.ui.Dialog({
		...
		primary_action(args){
			...
			return frappe.call({
				type: "GET",
				method: "snd_sbdma_stock.snd_sbdma_stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
