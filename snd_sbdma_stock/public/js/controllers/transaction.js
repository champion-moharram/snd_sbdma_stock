...
erpnext.TransactionController = class TransactionController extends erpnext.taxes_and_totals {
	setup() {
		...
		if (this.frm.fields_dict["items"].grid.get_field('blanket_order')) {
			this.frm.set_query("blanket_order", "items", function(doc, cdt, cdn) {
				...
				return {
					query: "snd_sbdma_stock.controllers.queries.get_blanket_orders",
					filters: {
						"company": doc.company,
	...
	make_payment_request() {
		...
		frappe.call({
			method:"snd_sbdma_stock.snd_sbdma_stock.doctype.payment_request.payment_request.make_payment_request",
			args: {
	...
	barcode(doc, cdt, cdn)  {
		...
		if (row.barcode) {
			snd_sbdma_stock.snd_sbdma_stock.utils.set_item_details_using_barcode(this.frm, row, (r) => {
				debugger
	...
	apply_default_taxes() {
		...
		if (taxes_and_charges_field) {
			return frappe.call({
				method: "snd_sbdma_stock.controllers.accounts_controller.get_default_taxes_and_charges",
				args: {
		...
		if(item.item_code || item.serial_no) {
			if(!this.validate_company_and_party()) {
				...
				return this.frm.call({
					method: "snd_sbdma_stock.snd_sbdma_stock.get_item_details.get_item_details",
					child: item,
					args: {
						doc: me.frm.doc,
						args: {
	...
	get_incoming_rate(item, posting_date, posting_time, voucher_type, company) {
		...
		frappe.call({
			method: 'snd_sbdma_stock.snd_sbdma_stock.utils.get_incoming_rate',
			args: {
				args: item_args
			},
			callback: function(r) {
		...
		var set_party_account = function(set_pricing) {
			if (["Sales Invoice", "Purchase Invoice"].includes(me.frm.doc.doctype)) {
				...
				if(party && me.frm.doc.company) {
					return frappe.call({
						method: "snd_sbdma_stock.accounts.party.get_party_account",
						args: {
							company: me.frm.doc.company,
	...
	posting_date() {
		...
		if (this.frm.doc.posting_date) {
			...
			if ((this.frm.doc.doctype == "Sales Invoice" && this.frm.doc.customer) ||
				...
				return frappe.call({
					method: "snd_sbdma_stock.accounts.party.get_due_date",
					args: {
	...
	get_exchange_rate(transaction_date, from_currency, to_currency, callback) {
		...
		return frappe.call({
			method: "snd_sbdma_stock.setup.utils.get_exchange_rate",
			args: {
	...
	uom(doc, cdt, cdn) {
		...
		if(item.item_code && item.uom) {
			return this.frm.call({
				method: "snd_sbdma_stock.snd_sbdma_stock.get_item_details.get_conversion_factor",
				args: {
					item_code: item.item_code,
					uom: item.uom
				},
	...
	service_start_date(frm, cdt, cdn) {
		...
		if(child.service_start_date) {
			frappe.call({
				"method": "snd_sbdma_stock.snd_sbdma_stock.get_item_details.calculate_service_end_date",
				args: {"args": child},
				callback: function(r) {
	...
	ignore_pricing_rule() {
		if(this.frm.doc.ignore_pricing_rule) {
			...
			return this.frm.call({
				method: "snd_sbdma_stock.snd_sbdma_stock.doctype.pricing_rule.pricing_rule.remove_pricing_rules",
	...
	apply_pricing_rule(item, calculate_taxes_and_totals) {
		...
		return this.frm.call({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.pricing_rule.pricing_rule.apply_pricing_rule",
	...
	apply_price_list(item, reset_plc_conversion) {
		...
		// `snd_sbdma_stock.snd_sbdma_stock.get_item_details.apply_price_list` is sensitive to its value
		...
		return this.frm.call({
			method: "snd_sbdma_stock.snd_sbdma_stock.get_item_details.apply_price_list",
			args: {	args: args, doc: me.frm.doc },
			callback: function(r) {
				if (!r.exc) {
					frappe.run_serially([
	...
	taxes_and_charges() {
		...
		if(this.frm.doc.taxes_and_charges) {
			return this.frm.call({
				method: "snd_sbdma_stock.controllers.accounts_controller.get_taxes_and_charges",
				args: {
	...
	update_item_tax_map() {
		...
		if (item_codes.length) {
			return this.frm.call({
				method: "snd_sbdma_stock.snd_sbdma_stock.get_item_details.get_item_tax_info",
				args: {
					company: me.frm.doc.company,
	...
	item_tax_template(doc, cdt, cdn) {
		...
		if(item.item_tax_template) {
			return this.frm.call({
				method: "snd_sbdma_stock.snd_sbdma_stock.get_item_details.get_item_tax_map",
				args: {
					company: me.frm.doc.company,
					item_tax_template: item.item_tax_template,
					as_json: true
	...
	make_quality_inspection() {
		...
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: function () {
				...
				frappe.call({
					method: "snd_sbdma_stock.controllers.stock_controller.make_quality_inspections",
					args: {
						doctype: me.frm.doc.doctype,
	...
	get_method_for_payment() {
		var method = "snd_sbdma_stock.snd_sbdma_stock.doctype.payment_entry.payment_entry.get_payment_entry";
		if(cur_frm.doc.__onload && cur_frm.doc.__onload.make_payment_via_journal_entry){
			if(['Sales Invoice', 'Purchase Invoice'].includes( cur_frm.doc.doctype)){
				method = "snd_sbdma_stock.snd_sbdma_stock.doctype.journal_entry.journal_entry.get_payment_entry_against_invoice";
			}else {
				method= "snd_sbdma_stock.snd_sbdma_stock.doctype.journal_entry.journal_entry.get_payment_entry_against_order";
			}
		}
	...
	set_query_for_batch(doc, cdt, cdn) {
		...
		if(!item.item_code) {
			...
			return {
				query : "snd_sbdma_stock.controllers.queries.get_batch_no",
				filters: filters
			}
		}
	}
	...
	set_query_for_item_tax_template(doc, cdt, cdn) {
		...
		if(!item.item_code) {
			...
			return {
				query: "snd_sbdma_stock.controllers.queries.get_tax_template",
				filters: filters
			}
		}
	}
	...
	payment_terms_template() {
		...
		if(doc.payment_terms_template && doc.doctype !== 'Delivery Note') {
			...
			frappe.call({
				method: "snd_sbdma_stock.controllers.accounts_controller.get_payment_terms",
				args: {
	...
	payment_term(doc, cdt, cdn) {
		...
		if(row.payment_term) {
			frappe.call({
				method: "snd_sbdma_stock.controllers.accounts_controller.get_payment_term_details",
				args: {
	...
	blanket_order(doc, cdt, cdn) {
		...
		if (item.blanket_order && (item.parenttype=="Sales Order" || item.parenttype=="Purchase Order")) {
			frappe.call({
				method: "snd_sbdma_stock.snd_sbdma_stock.get_item_details.get_blanket_order_details",
				args: {
					args:{
						item_code: item.item_code,
...
erpnext.apply_putaway_rule = (frm, purpose=null) => {
	...
	frappe.call({
		method: "snd_sbdma_stock.snd_sbdma_stock.doctype.putaway_rule.putaway_rule.apply_putaway_rule",
		args: {
			doctype: frm.doctype,
			items: frm.doc.items,
