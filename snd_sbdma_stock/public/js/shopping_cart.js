...
frappe.ready(function () {
	...
	// update user
	if (full_name) {
		$('.navbar li[data-label="User"] a').html(
			'<i class="fa fa-fixed-width fa fa-user"></i> ' + full_name
		);
	}
...
$.extend(shopping_cart, {
	show_shoppingcart_dropdown: function () {
		$(".shopping-cart").on("shown.bs.dropdown", function () {
			if (!$(".shopping-cart-menu .cart-container").length) {
				return frappe.call({
					method: "snd_sbdma_stock.e_commerce.shopping_cart.cart.get_shopping_cart_menu",
					callback: function (r) {
						if (r.message) {
	...
	update_cart: function (opts) {
		if (frappe.session.user === "Guest") {
			...
			frappe.call("snd_sbdma_stock.e_commerce.api.get_guest_redirect_on_action").then((res) => {
			...
			return frappe.call({
				type: "POST",
				method: "snd_sbdma_stock.e_commerce.shopping_cart.cart.update_cart",
				args: {
