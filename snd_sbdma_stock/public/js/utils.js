...
$.extend(erpnext.utils, {
	...
	get_terms: function (tc_name, doc, callback) { 
		if (tc_name) {
			return frappe.call({
				method: "snd_sbdma_stock.snd_sbdma_stock.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions",
				args: {
	...
	make_bank_account: function (doctype, docname) {
		frappe.call({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.bank_account.bank_account.make_bank_account",
			args: {
				doctype: doctype,
				docname: docname,
			},
	...
	add_dimensions: function (report_name, index) {
		...
		frappe.call({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.accounting_dimension.accounting_dimension.get_dimensions",
			callback: function (r) {
	...
	add_inventory_dimensions: function (report_name, index) {
		...
		frappe.call({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.inventory_dimension.inventory_dimension.get_inventory_dimensions",
			callback: function (r) {
	...
	make_pricing_rule: function (doctype, docname) {
		frappe.call({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.pricing_rule.pricing_rule.make_pricing_rule",
			args: {
				doctype: doctype,
				docname: docname,
			},
	...
	get_tree_options: function (option) {
		...
		if (user_permission && user_permission[unscrub_option]) {
			...
			let localOption = locals[`:${unscrub_option}`] || locals[unscrub_option];
			if (localOption) {
				options = $.map(localOption, function (c) {
					return c.name;
				}).sort();
			} else {
				// console.error(`Data for ${unscrub_option} not found in locals, fetching now...`);
				frappe.call({
					method: 'frappe.client.get_list',
					args: {
						doctype: unscrub_option,
						fields: ['name']
					},
					callback: function (r) {
						if (r.message) {
							locals[unscrub_option] = r.message.reduce((acc, doc) => {
								acc[doc.name] = doc;
								return acc;
							}, {});
							// Retry fetching tree options now that data is available
						}
					}
				});
				options = []; // Default to empty until data is fetched
			}
		}
	...
	get_fiscal_year: function (date, with_dates = false, boolean = false) {
		...
		let fiscal_year = "";
		frappe.call({
			method: "snd_sbdma_stock.accounts.utils.get_fiscal_year",
			args: {
				date: date,
				boolean: boolean,
			},
					...
					},
					{
						...
						onchange: function () {
							...
							if (item_code && warehouse) {
								frappe.call({
									method: "snd_sbdma_stock.snd_sbdma_stock.utils.get_latest_stock_qty",
									args: {
										item_code: item_code,
										warehouse: warehouse,
									},
						...
						get_query: (e) => {
							return {
								query: "snd_sbdma_stock.snd_sbdma_stock.doctype.item_alternative.item_alternative.get_alternative_items",
								filters: {
...
erpnext.utils.update_child_items = function (opts) {
	...
	const fields = [
		...
		},
		{
			...
			get_query: function () {
				...
				return {
					query: "snd_sbdma_stock.controllers.queries.item_query",
					filters: filters,
				};
			},
		},
	...
	new frappe.ui.Dialog({
		...
		primary_action: function () {
			...
			frappe.call({
				method: "snd_sbdma_stock.controllers.accounts_controller.update_child_qty_rate",
				freeze: true,
				args: {
	...
	$.each(frappe.boot.service_level_agreement_doctypes, function (_i, d) {
		frappe.ui.form.on(d, {
			onload: function (frm) {
				...
				frappe.call({
					method: "snd_sbdma_stock.snd_sbdma_stock.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters",
					args: {
