erpnext.SerialNoBatchSelector = class SerialNoBatchSelector {
	...
	make_dialog() {
		...
		let fields = [
			...
			},
			{
				...
				get_query: function () {
					return {
						query: "snd_sbdma_stock.controllers.queries.warehouse_query",
						filters: [
			...
			},
			{
				...
				click: () => {
					...
					let numbers = frappe.call({
						method: "snd_sbdma_stock.snd_sbdma_stock.doctype.serial_no.serial_no.auto_fetch_serial_number",
						args: {
							qty: qty,
				...
				fields: [
					{
						...
						change: function () {
							...
							if (me.warehouse_details.name) {
								frappe.call({
									method: "snd_sbdma_stock.snd_sbdma_stock.doctype.batch.batch.get_batch_qty",
									args: {
										batch_no,
	...
	get_serial_no_fields() {
		...
		if (me.frm.doc.doctype === "POS Invoice" && !this.showing_reserved_serial_nos_error) {
			frappe
				.call({
					method: "snd_sbdma_stock.snd_sbdma_stock.doctype.serial_no.serial_no.get_pos_reserved_serial_nos",
					args: {
						filters: {
							item_code: me.item_code,
							warehouse:
