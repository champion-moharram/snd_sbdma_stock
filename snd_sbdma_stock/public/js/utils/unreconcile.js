...
erpnext.accounts.unreconcile_payment = {
	add_unreconcile_btn(frm) {
		if (frm.doc.docstatus == 1) {
			...
			// frappe.call({
			// 	method: "snd_sbdma_stock.snd_sbdma_stock.doctype.unreconcile_payment.unreconcile_payment.doc_has_references",
			// 	args: {
	...
	build_unreconcile_dialog(frm) {
		if (
			...
			// get linked payments
			frappe.call({
				method: "snd_sbdma_stock.snd_sbdma_stock.doctype.unreconcile_payment.unreconcile_payment.get_linked_payments_for_doc",
				args: {
	...
	create_unreconcile_docs(selection_map) {
		frappe.call({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.unreconcile_payment.unreconcile_payment.create_unreconcile_doc_for_selection",
			args: {
