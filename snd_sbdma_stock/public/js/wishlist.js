...
$.extend(wishlist, {
	...
	add_remove_from_wishlist(action, args, success_action, failure_action, async = false) {
		...
		if (frappe.session.user === "Guest") {
			...
			let method = "snd_sbdma_stock.snd_sbdma_stock.doctype.wishlist.wishlist.add_to_wishlist";
			if (action === "remove") {
				method = "snd_sbdma_stock.snd_sbdma_stock.doctype.wishlist.wishlist.remove_from_wishlist";
			}
