				...
				callback: (r) => {
					...
					rows.find(".btn-move").on("click", function () {
						...
						frappe.prompt(
							...
							(data) => {
								frappe.call({
									method: "snd_sbdma_stock.snd_sbdma_stock.doctype.stock_entry.stock_entry_utils.make_stock_entry",
									args: {
					...
					rows.find(".btn-split").on("click", function () {
						...
						frappe.prompt(
							...
							(data) => {
								frappe
									.xcall("snd_sbdma_stock.snd_sbdma_stock.doctype.batch.batch.split_batch", {
										item_code: frm.doc.item,
										batch_no: frm.doc.name,
										qty: data.qty,
