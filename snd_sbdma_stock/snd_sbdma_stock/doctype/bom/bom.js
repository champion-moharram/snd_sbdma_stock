...
frappe.ui.form.on("BOM", {
	setup(frm) {
		...
		frm.set_query("item", function () {
			return {
				query: "snd_sbdma_stock.snd_sbdma_stock.doctype.bom.bom.item_query",
				filters: {
					is_stock_item: 1,
				},
			};
	...
	make_work_order(frm) {
		frm.events.setup_variant_prompt(frm, "Work Order", (frm, item, data, variant_items) => {
			frappe.call({
				method: "snd_sbdma_stock.snd_sbdma_stock.doctype.work_order.work_order.make_work_order",
				args: {
					bom_no: frm.doc.name,
					item: item,
					qty: data.qty || 0.0,
	...
	make_variant_bom(frm) {
		frm.events.setup_variant_prompt(
			...
			(frm, item, data, variant_items) => {
				frappe.call({
					method: "snd_sbdma_stock.snd_sbdma_stock.doctype.bom.bom.make_variant_bom",
					args: {
	...
	make_quality_inspection(frm) {
		frappe.model.open_mapped_doc({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.quality_inspection.quality_inspection.make_quality_inspection",
			frm: frm,
		});
	},
