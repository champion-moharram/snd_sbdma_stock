...
frappe.ui.form.on("Pick List", {
	setup: (frm) => {
		...
		frm.set_query("work_order", () => {
			return {
				query: "snd_sbdma_stock.snd_sbdma_stock.doctype.pick_list.pick_list.get_pending_work_orders",
				filters: {
					company: frm.doc.company,
				},
			};
	...
	refresh: (frm) => {
		...
		if (frm.doc.docstatus === 1) {
			frappe
				.xcall("snd_sbdma_stock.snd_sbdma_stock.doctype.pick_list.pick_list.target_document_exists", {
				...
				frappe.prompt(
					...
					(data) => {
						...
						erpnext.utils.map_current_doc({
							method: "snd_sbdma_stock.snd_sbdma_stock.doctype.work_order.work_order.create_pick_list",
							target: frm,
	...
	create_delivery_note: (frm) => {
		frappe.model.open_mapped_doc({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.pick_list.pick_list.create_delivery_note",
			frm: frm,
		});
	},
	create_stock_entry: (frm) => {
		frappe
			.xcall("snd_sbdma_stock.snd_sbdma_stock.doctype.pick_list.pick_list.create_stock_entry", {
				pick_list: frm.doc,
			})
			.then((stock_entry) => {
	...
	add_get_items_button: (frm) => {
		...
		frm.get_items_btn = frm.add_custom_button(__("Get Items"), () => {
			erpnext.utils.map_current_doc({
				method: "snd_sbdma_stock.snd_sbdma_stock.doctype.sales_order.sales_order.create_pick_list",
...
function get_item_details(item_code, uom = null) {
	if (item_code) {
		return frappe.xcall("snd_sbdma_stock.snd_sbdma_stock.doctype.pick_list.pick_list.get_item_details", {
			item_code,
			uom,
		});
	}
