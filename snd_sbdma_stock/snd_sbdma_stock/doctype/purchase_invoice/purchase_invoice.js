...
erpnext.accounts.PurchaseInvoice = class PurchaseInvoice extends erpnext.buying.BuyingController {
	...
	refresh(doc) {
		...
		if(doc.docstatus===0) {
			...
			this.frm.add_custom_button(__('Purchase Receipt'), function() {
				erpnext.utils.map_current_doc({
					method: "snd_sbdma_stock.snd_sbdma_stock.doctype.purchase_receipt.purchase_receipt.make_purchase_invoice",
	...
	unblock_invoice() {
		const me = this;
		frappe.call({
			'method': 'snd_sbdma_stock.snd_sbdma_stock.doctype.purchase_invoice.purchase_invoice.unblock_invoice',
	...
	make_comment_dialog_and_block_invoice(){
		...
		this.dialog.set_primary_action(__('Save'), function() {
			...
			frappe.call({
				'method': 'snd_sbdma_stock.snd_sbdma_stock.doctype.purchase_invoice.purchase_invoice.block_invoice',
				'args': {
	...
	set_release_date(data) {
		return frappe.call({
			'method': 'snd_sbdma_stock.snd_sbdma_stock.doctype.purchase_invoice.purchase_invoice.change_release_date',
			'args': data,
	...
	supplier() {
		...
		erpnext.utils.get_party_details(this.frm, "snd_sbdma_stock.accounts.party.get_party_details",
			{
	...
	make_inter_company_invoice(frm) {
		frappe.model.open_mapped_doc({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.purchase_invoice.purchase_invoice.make_inter_company_sales_invoice",
			frm: frm
		});
	}
	...
	make_debit_note() {
		frappe.model.open_mapped_doc({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.purchase_invoice.purchase_invoice.make_debit_note",
			frm: cur_frm
		})
	}
};
...
frappe.ui.form.on("Purchase Invoice", {
	...
	make_purchase_receipt: function(frm) {
		frappe.model.open_mapped_doc({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.purchase_invoice.purchase_invoice.make_purchase_receipt",
			frm: frm,
	...
	company: function(frm) {
		...
		if (frm.doc.company) {
			frappe.call({
				method:
					"snd_sbdma_stock.accounts.party.get_party_account",
				args: {
					party_type: 'Supplier',
