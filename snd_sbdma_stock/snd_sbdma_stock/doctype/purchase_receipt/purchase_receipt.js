	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus === 1 && frm.doc.is_internal_supplier && !frm.doc.inter_company_reference) {
			frm.add_custom_button(__('Delivery Note'), function() {
				frappe.model.open_mapped_doc({
					method: 'snd_sbdma_stock.snd_sbdma_stock.doctype.purchase_receipt.purchase_receipt.make_inter_company_delivery_note',
					frm: cur_frm,
				})
			}, __('Create'));
		}
...
erpnext.stock.PurchaseReceiptController = class PurchaseReceiptController extends erpnext.buying.BuyingController {
	...
	make_purchase_return() {
		frappe.model.open_mapped_doc({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.purchase_receipt.purchase_receipt.make_purchase_return",
			frm: cur_frm
		})
	}
...
cur_frm.cscript.update_status = function(status) {
	...
	frappe.call({
		method:"snd_sbdma_stock.snd_sbdma_stock.doctype.purchase_receipt.purchase_receipt.update_purchase_receipt_status",
...
cur_frm.cscript['Make Stock Entry'] = function() {
	frappe.model.open_mapped_doc({
		method: "snd_sbdma_stock.snd_sbdma_stock.doctype.purchase_receipt.purchase_receipt.make_stock_entry",
		frm: cur_frm,
	})
}
