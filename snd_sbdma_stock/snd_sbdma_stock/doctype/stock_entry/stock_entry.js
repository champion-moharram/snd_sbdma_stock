...
{% include 'snd_sbdma_stock/snd_sbdma_stock/landed_taxes_and_charges_common.js' %};
...
frappe.ui.form.on('Stock Entry', {
	...
	setup_quality_inspection: function(frm) {
		...
		frm.set_query("quality_inspection", "items", function(doc, cdt, cdn) {
			...
			return {
				query:"snd_sbdma_stock.snd_sbdma_stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	refresh: function(frm) {
			...
			if (frm.doc.add_to_transit && frm.doc.purpose=='Material Transfer' && frm.doc.per_transferred < 100) {
				frm.add_custom_button(__('End Transit'), function() {
					frappe.model.open_mapped_doc({
						method: "snd_sbdma_stock.snd_sbdma_stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm
					})
				});
			}
		...
		if (frm.doc.docstatus === 0) {
			frm.add_custom_button(__('Purchase Invoice'), function() {
				erpnext.utils.map_current_doc({
					method: "snd_sbdma_stock.snd_sbdma_stock.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
			...
			frm.add_custom_button(__('Material Request'), function() {
				...
				const d = erpnext.utils.map_current_doc({
					method: "snd_sbdma_stock.snd_sbdma_stock.doctype.material_request.material_request.make_stock_entry",
		...
		if (frm.doc.docstatus === 0 && frm.doc.purpose == "Material Issue") {
			frm.add_custom_button(__('Expired Batches'), function() {
				frappe.call({
					method: "snd_sbdma_stock.snd_sbdma_stock.doctype.stock_entry.stock_entry.get_expired_batch_items",
					callback: function(r) {
						if (!r.exc && r.message) {
							frm.set_value("items", []);
	...
	validate_purpose_consumption: function(frm) {
		frappe.call({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
		}).then(r => {
			if (cint(r.message) == 0
	...
	set_serial_no: function(frm, cdt, cdn, callback) {
		...
		frappe.call({
			method: "snd_sbdma_stock.snd_sbdma_stock.get_item_details.get_serial_no",
			args: {"args": args},
			callback: function(r) {
				if (!r.exe && r.message){
	...
	make_retention_stock_entry: function(frm) {
		frappe.call({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.stock_entry.stock_entry.move_sample_to_retention_warehouse",
			args:{
	...
	get_warehouse_details: function(frm, cdt, cdn) {
		...
		if(!child.bom_no) {
			frappe.call({
				method: "snd_sbdma_stock.snd_sbdma_stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					"args": {
	...
	get_items_from_bom: function(frm) {
		...
		d.get_input("fetch").on("click", function() {
			...
			frappe.call({
				method: "snd_sbdma_stock.snd_sbdma_stock.doctype.bom.bom.get_bom_items",
				args: values,
				callback: function(r) {
					if (!r.message) {
	...
	purchase_order: (frm) => {
		if (frm.doc.purchase_order) {
			...
			erpnext.utils.map_current_doc({
				method: 'snd_sbdma_stock.snd_sbdma_stock.doctype.stock_entry.stock_entry.get_items_from_subcontract_order',
...
frappe.ui.form.on('Stock Entry Detail', {
	...
	uom: function(doc, cdt, cdn) {
		...
		if(d.uom && d.item_code){
			return frappe.call({
				method: "snd_sbdma_stock.snd_sbdma_stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty
...
var validate_sample_quantity = function(frm, cdt, cdn) {
	...
	if (d.sample_quantity && frm.doc.purpose == "Material Receipt") {
		frappe.call({
			method: 'snd_sbdma_stock.snd_sbdma_stock.doctype.stock_entry.stock_entry.validate_sample_quantity',
			args: {
				batch_no: d.batch_no,
				item_code: d.item_code,
...
erpnext.stock.StockEntry = class StockEntry extends erpnext.stock.StockController {
	...
	set_default_account(callback) {
		...
		if(this.frm.doc.company && erpnext.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			return this.frm.call({
				method: "snd_sbdma_stock.accounts.utils.get_company_default",
				args: {
	...
	work_order() {
		...
		return frappe.call({
			method: "snd_sbdma_stock.snd_sbdma_stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
