...
from snd_sbdma_stock.snd_sbdma_stock.doctype.bin.bin import update_qty as update_bin_qty
from snd_sbdma_stock.snd_sbdma_stock.doctype.inventory_dimension.inventory_dimension import get_inventory_dimensions
from snd_sbdma_stock.snd_sbdma_stock.utils import (
...
from snd_sbdma_stock.snd_sbdma_stock.valuation import FIFOValuation, LIFOValuation, round_off_if_near_zero
...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	args:
	        ...
	        such cases certain validations need to be ignored (like negative
	                        stock)
	"""
	from snd_sbdma_stock.controllers.stock_controller import future_sle_exists
...
class update_entries_after:
	...
	def get_incoming_outgoing_rate_from_transaction(self, sle):
		...
		# Sales and Purchase Return
			...
			if frappe.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return"):
				from snd_sbdma_stock.controllers.sales_and_purchase_return import (
	...
	def get_fallback_rate(self, sle) -> float:
		...
		return get_valuation_rate(
			...
			currency=snd_sbdma_stock.get_company_currency(sle.company),
			company=sle.company,
			batch_no=sle.batch_no,
		)
...
def get_valuation_rate(
	...
	if (
		...
		and cint(snd_sbdma_stock.is_perpetual_inventory_enabled(company))
	):
